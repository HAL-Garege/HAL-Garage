...
(() => {
  ...
  async function findSaleId(card){
    ...
    const raw=[photo?.getAttribute('onclick')||'',photo?.dataset?.saleId||'',photo?.dataset?.sale||'',card.dataset?.saleId||''].join(' ');
  ...
  function getSaleCards(){
    ...
    app.querySelectorAll('*').forEach(el=>{
      if(!el.children.length&&/Venta\s*#\s*\d+/i.test(el.textContent||'')){
        let card=el.closest('.result,.card');
        if(!card)card=el.parentElement?.parentElement;
        if(card&&!found.has(card)){found.add(card);cards.push(card)}
      }
    });
    return cards;
  }
  function inject(){
    if(!canEdit())return;
    getSaleCards().forEach(card=>{
      if(card.querySelector('[data-hal-edit-sale]'))return;
      const photo=[...card.querySelectorAll('button')].find(b=>/ver fotos/i.test(b.textContent||''));
  ...
  hookHistory();
  [150,500,1000,1800,3000].forEach(ms=>setTimeout(()=>{hookHistory();inject()},ms));
})();