...
(() => {
  ...
  function close(){document.getElementById('halEditSale')?.remove()}
  async function open(id){
    ...
    try{
      const [{data:s,error:se},{data:items,error:ie},{data:pays,error:pe}]=await Promise.all([
        db.from('sales').select('id,sale_number,total,status,vehicle_id,vehicles(plate,vehicle_type_id),clients(full_name)').eq('id',id).single(),
      ...
      const itemHtml=(items||[]).map((x,i)=>{const name=x.service_name_snapshot||'';return `<div class="result" data-item="${x.id}"><label>Servicio ${i+1}</label><select class="hal-es-service">${opts}</select><label>Cantidad</label><input class="hal-es-qty" type="number" min="1" step="1" value="${Number(x.quantity||1)}"><div class="muted">Precio actual según vehículo: <span class="hal-es-price">${money2(x.price_applied)}</span></div></div>`}).join('');
      setTimeout(()=>{(items||[]).forEach((x,i)=>{const r=document.querySelectorAll('#halEditSale .result')[i],sel=r?.querySelector('.hal-es-service');if(sel){const byName=services2.find(z=>z.name.trim().toLowerCase()===String(x.service_name_snapshot||'').trim().toLowerCase());sel.value=byName?.id||x.service_id||services2[0]?.id||''} updatePrices()})},0);
      ...
      document.querySelectorAll('#halEditSale .hal-es-service,#halEditSale .hal-es-qty').forEach(e=>e.addEventListener('change',updatePrices));
      function updatePrices(){
        const vt=s.vehicles?.vehicle_type_id;let total=0;document.querySelectorAll('#halEditSale .result').forEach(r=>{const sid=r.querySelector('.hal-es-service')?.value,q=Math.max(1,Number(r.querySelector('.hal-es-qty')?.value||1));const p=prices2.find(z=>z.service_id===sid&&z.vehicle_type_id===vt);const price=Number(p?.price||0);r.querySelector('.hal-es-price').textContent=money2(price);total+=price*q});const t=document.getElementById('hal-es-total');if(t)t.textContent=money2(total)}
      window.__halEditSale={s,items,services2,prices2,updatePrices};updatePrices();
    }catch(e){toast(e.message||'No se pudo abrir la venta.',true)}
  }
  window.halCloseEditSale=close;
  window.halSaveEditSale=async id=>{try{const st=window.__halEditSale;if(!st)return;const method=document.getElementById('hal-es-payment').value;const rows=[...document.querySelectorAll('#halEditSale .result')];let total=0;for(const r of rows){const iid=r.dataset.item,sid=r.querySelector('.hal-es-service').value,q=Math.max(1,parseInt(r.querySelector('.hal-es-qty').value||1,10)),p=st.prices2.find(z=>z.service_id===sid&&z.vehicle_type_id===st.s.vehicle_type_id);if(!p)throw new Error('No existe precio para ese servicio y tipo de vehículo.');const price=Number(p.price),sub=price*q;total+=sub;const svc=st.services2.find(x=>x.id===sid);const {error}=await db.from('sale_items').update({service_id:sid,vehicle_type_id:st.s.vehicle_type_id,service_name_snapshot:svc?.name||'',price_applied:price,quantity:q,subtotal:sub}).eq('id',iid);if(error)throw error}
      let r=await db.from('sales').update({total}).eq('id',id);if(r.error)throw r.error;
      const p=await db.from('payments').update({method,amount:total}).eq('sale_id',id);if(p.error)throw p.error;
      const cm=await db.from('cash_movements').update({payment_method:method,amount:total}).eq('sale_id',id);if(cm.error)console.warn(cm.error);
      close();toast('Venta actualizada correctamente');if(typeof clientHistory==='function'){const cid=st.s.client_id; if(cid)await clientHistory(cid)}
  ...
  document.addEventListener('click',e=>{if(e.target?.textContent?.match(/Historial de servicios/))setTimeout(inject,100)});
