// HAL Garage — edición de ventas v3.
// El botón se inserta automáticamente cada vez que aparece el historial.
(() => {
  ...
  async function editSale(id){
    ...
    try{
      const {data:sale,error}=await db.from('sales').select('id,sale_number,client_id,vehicle_id,total,status,created_at').eq('id',id).single();
      if(error)throw error;
      if(sale.status==='voided')return toast('Esta venta no se puede editar.',true);
      const {data:items,error:ie}=await db.from('sale_items').select('id,service_id,vehicle_type_id,service_name_snapshot,price_applied,quantity,subtotal').eq('sale_id',id).order('id');
      if(ie)throw ie;
      const {data:payment,error:pe}=await db.from('payments').select('id,payment_method,amount').eq('sale_id',id).limit(1).maybeSingle();
      if(pe)throw pe;
      ...
      document.getElementById('editService').value=item?.service_id||'';
      selectEditPayment(payment?.payment_method||'cash');
  ...
  async function saveEditSale(){
    ...
    if(!service||!item)return toast('No se encontró el detalle de la venta.',true);
    if(!(amount>0))return toast('Ingresa un costo válido.',true);
    try{
      const subtotal=amount*Number(item.quantity||1);
      let r=await db.from('sale_items').update({service_id:service.id,service_name_snapshot:service.name,price_applied:amount,subtotal}).eq('id',item.id);if(r.error)throw r.error;
      r=await db.from('sales').update({total:subtotal}).eq('id',x.sale.id);if(r.error)throw r.error;
      if(x.payment){r=await db.from('payments').update({payment_method:pay,amount:subtotal}).eq('id',x.payment.id);if(r.error)throw r.error}
      else{r=await db.from('payments').insert({sale_id:x.sale.id,payment_method:pay,amount:subtotal,...createdBy()});if(r.error)throw r.error}
      r=await db.from('cash_movements').select('id').eq('reference_id',x.sale.id).eq('movement_type','income').limit(1).maybeSingle();if(r.error)throw r.error;
      if(r.data){r=await db.from('cash_movements').update({amount:subtotal,payment_method:pay}).eq('id',r.data.id);if(r.error)throw r.error}
      const cid=x.sale.client_id;closeEditSale();toast('Venta actualizada correctamente');
      if(cid&&typeof clientHistory==='function')setTimeout(()=>clientHistory(cid),80);
  ...
  async function addEditButtons(){
    try{
      ...
      const nums=cards.map(card=>(card.textContent||'').match(/Venta\s*#\s*([^\s]+)/i)?.[1]).filter(Boolean);
      if(!nums.length)return;
      const {data:sales,error}=await db.from('sales').select('id,sale_number,status').in('sale_number',nums);
      if(error)throw error;
      const byNumber=Object.fromEntries((sales||[]).map(s=>[String(s.sale_number),s]));
      cards.forEach(card=>{
        const m=(card.textContent||'').match(/Venta\s*#\s*([^\s]+)/i);if(!m)return;
        const sale=byNumber[m[1]];if(!sale)return;
        if(sale.status==='voided'){card.dataset.halEditSale='1';return;}
        if(card.querySelector('[data-hal-edit-button]')){card.dataset.halEditSale='1';return;}
        const b=document.createElement('button');b.className='btn alt';b.textContent='✏️ Editar';b.setAttribute('data-hal-edit-button','1');b.style.cssText='width:auto;margin:7px 0 0';b.onclick=()=>editSale(sale.id);card.appendChild(b);card.dataset.halEditSale='1';
      });
    }catch(e){console.warn('Editar ventas v3:',e)}
  }
  ...
  if(app){
    const observer=new MutationObserver(()=>{clearTimeout(window.__halEditTimer);window.__halEditTimer=setTimeout(addEditButtons,80)});
    observer.observe(app,{childList:true,subtree:true});
    setTimeout(addEditButtons,250);
  }
})();
