(async()=>{
  window.clientHistory=async function(clientId){
    ...
    try{
      const sales=await q('sales',db.from('sales').select('id,sale_number,total,status,created_at,service_date,vehicle_id').eq('client_id',clientId).order('service_date',{ascending:false}).order('created_at',{ascending:false}));
      ...
      const fmtSaleDate=x=>x.service_date?new Date(x.service_date+'T12:00:00').toLocaleDateString('es-PE'):new Date(x.created_at).toLocaleDateString('es-PE');
      ...
      ${sales.map(x=>{const v=vehicles.find(v=>v.id===x.vehicle_id),its=items.filter(i=>i.sale_id===x.id);return `<div class="card"><div class="row"><b>Venta #${esc(x.sale_number)}</b><span class="badge">${fmtSaleDate(x)}</span></div><div class="muted">${v?esc(v.plate):'Vehículo'} · ${esc(x.status)}</div>${its.map(i=>`<div class="result row"><span>${esc(i.service_name_snapshot)} × ${i.quantity}</span><b>${money(i.subtotal)}</b></div>`).join('')}<div class="row" style="margin-top:8px"><span>Total</span><b>${money(x.total)}</b></div><button class="btn alt" style="width:auto;margin-top:8px" onclick="editSale('${x.id}')">✏️ Editar</button></div>`}).join('')||'<div class="card muted">Este cliente todavía no tiene servicios registrados.</div>'}`);
