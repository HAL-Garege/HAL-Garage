...
(() => {
  const install = () => {
    ...
    window.morePage = async function () {
      ...
      card.innerHTML = `<b>⭐ Club HAL Garage</b><div class="muted" style="margin-top:6px">Administración del programa de puntos</div><button class="btn" onclick="openClubCampaignsAdmin()">⭐ Campañas de puntos</button>`;
    ...
    window.openClubCampaignsAdmin = () => {
      ...
      window.open('./club-hal-garage/admin-campanas.html?role='+encodeURIComponent(role()), '_blank', 'noopener');
    };
  };
  install();
  const timer = setInterval(() => { install(); if (window.__halClubCampaignsInstalled) clearInterval(timer); }, 250);
